...
canv.id = 'canvas';
const mouseHoverEffect = document.getElementById('mouse-hover-effect');
if (mouseHoverEffect) {
    mouseHoverEffect.appendChild(canv);
    document.addEventListener('DOMContentLoaded', function() {
        const settingHoverMouse = document.getElementById('setting-mouse-hover');
        var rippleSettings;
        if (settingHoverMouse) {
            const size = parseInt(settingHoverMouse.getAttribute('size'));
            const color = settingHoverMouse.getAttribute('color').split(",");
            const speed = parseInt(settingHoverMouse.getAttribute('speed'));
            rippleSettings = {
                maxSize: size ? size : 100,
                animationSpeed: speed ? speed : 5,
                strokeColor: color ? color : [148, 217, 255],
            }
        } else {
            rippleSettings = {
                maxSize: 100,
